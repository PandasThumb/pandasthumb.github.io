...
Night of the Open Door at Arizona State University (ASU) is an annual event that allows the general public to visit any of the five ASU campuses and get an inside peek to what ASU students see, do, and learn everyday.  There are hundreds of interactive activities that visitors can participate in and thousands of students, faculty, and staff to teach them anything from science to technology to engineering to arts to math.  Learn more about it [here](https://opendoor.asu.edu/).  Specifically at the BioDesign Institute, visitors could learn things about viruses to whale poop.  The activities our lab put together was titled “Disease Detectives.”  There were four main stations: Protein Puzzle, Disease Detectives, Seq N Destroy, and Next Gen Sequencing.  
...
The Next Gen Sequencing was an activity where a visitor learned about how DNA sequences are read and recorded.  A volunteer would demonstrate using the DNA strands and individual nucleotides to explain the activity.  They would start by explaining that DNA is double stranded and each strand is the mirror image of the other.  Only short sequences are read at one time.  The different strands of DNA are tagged and one strand binds to a corresponding tag in the flow cell.  The DNA bends over so the other tags bind and it forms a bridge.  Next, a polymerase is added and a copy of the DNA is made.  Chemicals are added to make the bridge come apart.  We only want to read one strand at a time or we won’t know the correct sequence because each is different.  So we wash away all of the strands that have a certain tag.  To read the DNA, we throw out a bunch of nucleotides one letter at a time and see which one sticks to our piece of DNA.  Nucleotide binding to the DNA has to go in order starting at the top.  Because A only binds to T and G only binds to C, we can tell when a nucleotide binds to another one because it will light up.  We can then record the color light it makes to know which letter is next in our sequence.  Not the whole strand can be read this way because when there gets to be too many bases added, the chain becomes unstable and falls apart.  But we’ve only read a little more than half the sequence.  The way to read the rest is to go from the other direction.  So we wash away the bound nucleotides so it’s only one strand again.  A new bridge is formed and another copy of the DNA is made.  This time we wash away all of the sequences we’ve already started to read and throw more nucleotides at the strand to determine the sequence.  To find the full sequence, we find where each sequence overlaps and that gives us the full read.
