...
In opposition to BS, not only did our populations not go extinct, but actually grew in size (Figure 3). While the vast majority of mutations were deleterious (and many fixed!), the overwhelming preponderance of mutations that fixed were beneficial due to positive selection. This is because the probability of fixation of beneficial mutations always exceeds that of deleterious, even if they are approximately equal as &vert;s&vert; = 1/(2N), where s is the selection coefficient and N is the population size.
