...
<p>&nbsp;</p>
_This is part 2 of a series of 7 posts by Lars Johan Erkell, with comments on each by Ola Hössjer and a reply by Erkell.  [An eighth post by Erkell](https://pandasthumb.org/archives/2022/11/erkell8.html) is near this one, put here because he refers to it. Part 1 will be found [here](http://pandasthumb.org/archives/2022/11/erkell1.html)._
...
In order to be able to estimate the probability that a biological system should have arisen by chance (that is, step (a) of the fine-tuning argument) a naturalistic model M2 is required that describes how the system came about or evolved. The fact that the model is naturalistic means that it only involves natural explanatory mechanisms. When it comes to biological evolution, i.e. the development of complicated life forms from simple single-celled organisms, the five naturalistic mechanisms are usually involved (i) mutations, (ii) genetic drift, i.e. random variation in reproductivity when parents pass on their DNA, (iii) natural selection, (iv) recombinations of genetic material from a pair of parents, and v) the mixing of gene pools caused by geographic migration.

Erkell claims that it is practically impossible to give a reasonable estimate of the probability that, for example, a protein could have evolved in accordance with a naturalistic model M2. I agree that this is extremely difficult, since such a model must necessarily be complicated, with more or less unknown parameters. Nevertheless, I believe that it is possible to demonstrate the difficulty of naturalistic processes to produce complex structures. If one only focuses on certain properties of the protein, that according to M2 must have evolved, a conservative upper bound can be found for the probability that the protein came about through evolutionary processes. In addition, it is possible to account for the fact that several of the model parameters (such as selection pressure, mutation rate, population sizes, generation times, etc.) are difficult to estimate from data with good accuracy, by performing a so-called sensitivity analysis in order to calculate how the upper bound of the probability for the protein to evolve, changes when the constituent parameters change. If this upper bound of the probability is small for all biologically reasonable values of the model parameters, an argument has been made for the difficulty of the proposed naturalistic model M2 to explain the origin of the protein of interest. I therefore argue, contrary to Erkell, that the first step (a) of our method for detecting fine-tuning is highly feasible in practice.

The second part (b) of our fine-tuning argument consists in finding a measure f(x) of how specified a biological system x is. As mentioned in the introduction, f(x) focuses on some characteristic that the observer recognizes, which in many cases brings to mind that an intelligent designer is behind the origin of the system. In TH2020 we give several examples of how the system x and its specification f(x) can be chosen. For instance, x may correspond to a protein or a molecular machine, with a specification that tells how many parts that must be in place for x to function. Another example is that f(x) indicates the biological fitness of an organism x, that is, its reproductive capacity. In connection with the latter example, we argue in TH2020 that evolution will eventually break down the genetic material of a species because most mutations are harmful or neutral. The consequence is that the fitness of individuals of the species decreases over time. This in turn means that the probability in (a) becomes very low for an evolutionary model M2, based on the aforementioned mechanisms (i)-(v), to be able to explain that the organism in question arose during millions of years of development, because its genetic material during such a long time would be depleted.

But if a naturalistic model M2 has difficulties explaining the emergence of a biological system that contains some form of specification, the question remains whether there is any other better explanatory model. At the end of TH2020, we give examples of design- or creationist-inspired models M1 that we believe have greater possibilities than M2 to explain the emergence of the fine-tuned system. To make such reasoning more rigorous, it is necessary to show that the probability of such a system to emerge is much higher for the design model M1 than for the naturalistic model M2. In the example above, where f(x) corresponds to the biological fitness of an organism x, we refer to various articles where a design model is described, which, in addition to the aforementioned (micro)evolutionary mechanisms (i)-(v) adds a sixth mechanism (vi) in the form of genetic variation in a first created pair, from which all individuals in the species descend. According to such a design model M1, the emergence of species can be explained by a combination of created variation (mechanism (vi)) and short-term, small-scale microevolution (mechanisms (i)-(v)). The problem of genetic depletion is then greatly reduced, since created variation makes it possible to assume that the species arose relatively recently, so that the genetic erosion, caused by the harmful mutations, acted over a much shorter time.sup>2</sup>.

Erkell is strongly critical of us introducing a design model to explain the emergence of a biological system. He writes:

> In order to be able to calculate the probability of how a complex biological structure can arise, we must therefore have a credible and detailed model of the process. This also applies if there is conscious design behind it. We must therefore have knowledge of the designer in order to assess the probability that the person concerned wanted his design exactly the way we see it to be. This requires detailed knowledge – we need to understand exactly how the designer thinks, what intentions, plans and means they have. In order to make statistical calculations, we must have concrete data, numbers.
>
...
I completely agree with Erkell that it is difficult to determine exactly what plans or intentions a designer has. But this is not necessary to know. It is enough to let design-inspired thinking generate concrete falsifiable hypotheses.  It is therefore possible to combine a design-inspired model with the hypothetico-deductive method, which forms the basis of empirical science, according to the following three steps:

1) Propose a falsifiable hypothesis, which may involve both natural mechanisms and design-inspired parts.

2.	Evaluate (through deduction and induction) and check if the hypothesis in (1) is consistent with the data. 

3) If the agreement in (2) is not good, go back to (1) and propose a better hypothesis.

Erkell makes very far-reaching, not to say unreasonable, demands on the design model, because he does not think that falsifiable hypotheses are enough. He also wants to include characteristics of the designer into the model, that often can be very difficult to test scientifically. Although it is interesting to have a separate conversation about the intentions of the designer, this has rather to do with the interpretation of and the reason for the emergence of the testable hypotheses, which is not required for the hypothetico-deductive method. One can thus use more or less intuitive design arguments to formulate testable hypotheses, without having to test the reasoning that led to the formulation of these hypotheses.

Erkell's reasoning also backfires like a boomerang on himself. Shouldn't he make the same rigorous demands on naturalistic models M2, where the interpretation of these models is included? How, for example, should randomness of an evolutionary model be explained? Is it epistemic chance (as a way of describing lack of knowledge), ontological chance (genuine non-determinism) or should quantum mechanics be used to explain the emergence of chance? And in the latter case, which interpretation of quantum mechanics should be used?
